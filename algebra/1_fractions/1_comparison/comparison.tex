\documentclass[a4paper, 12pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[T2A]{fontenc}
\usepackage[english,russian]{babel} 
\usepackage[left=20mm, top=20mm, right=20mm, bottom=20mm, nohead, includefoot]{geometry} 
\usepackage{amsmath,amsfonts,amssymb} % математический пакет
\usepackage{fancybox,fancyhdr} 
\usepackage{lastpage}
\usepackage{hyperref}
\usepackage{xcolor}
\hypersetup{
    colorlinks,
    linkcolor={red!50!black},
    citecolor={blue!50!black},
    urlcolor={blue!80!black}
}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[R]{}
\headsep=10mm 
\usepackage{xcolor}
\setlength{\footskip}{12.0pt}
\setlength{\headheight}{12.0pt}
\newcommand{\lr}[1]{\left({#1}\right)} % команда для скобок
\rfoot{Page \thepage \hspace{1pt} of \pageref{LastPage}}

\fancyfoot[C]{Made by \href{https://kharitonovegor.xyz}{Egor Kharitonov} with \LaTeX} 
\fancyhead[C]{{\Large \textbf{Math for Kristina. Fractions. Summer 2022. Comparison-1.}}}

\begin{document}

	{\large Please compare numbers.}	
	
	\section* {{\Large Easy}}
	
	{\Large \textbf{1.} $\dfrac{2}{2}\vee\dfrac{3}{2}$ \hspace{0.6cm}
		   \textbf{2.} $\dfrac{2}{5}\vee\dfrac{8}{5}$ \hspace{0.6cm}
		   \textbf{3.} $\dfrac{5}{3}\vee\dfrac{1}{3}$ \hspace{0.6cm}
		   \textbf{4.} $\dfrac{1}{8}\vee\dfrac{3}{8}$ \hspace{0.6cm}
           \textbf{5.} $\dfrac{5}{6}\vee\dfrac{2}{6}$
	
	\section* {{\Large Middle}}
	
	{\Large \textbf{1.} $\dfrac{3}{2}\vee\dfrac{1}{6}$ \hspace{0.6cm}
		   \textbf{2.} $\dfrac{2}{7}\vee\dfrac{2}{21}$ \hspace{0.6cm}
		   \textbf{3.} $\dfrac{5}{2}\vee\dfrac{5}{12}$ \hspace{0.6cm}
		   \textbf{4.} $\dfrac{1}{8}\vee\dfrac{3}{56}$ \hspace{0.6cm}
		   \textbf{5.} $\dfrac{2}{9}\vee\dfrac{4}{81}$ \\ [0.6cm]
		   \textbf{6.} $\dfrac{1}{2}\vee\dfrac{2}{7}$ \hspace{0.6cm}
		   \textbf{7.} $\dfrac{4}{8}\vee\dfrac{1}{7}$ \hspace{0.6cm}
		   \textbf{8.} $\dfrac{2}{12}\vee\dfrac{12}{5}$ \hspace{0.6cm}
		   \textbf{9.} $\dfrac{2}{7}\vee\dfrac{4}{5}$  \hspace{0.6cm}
		   \textbf{10.} $\dfrac{1}{19}\vee\dfrac{5}{9}$ \\ [0.6cm]}
	   
	 {\large $\vee$ symbol can stand for: $\textgreater$, $\textless$ or $=$.}
		
\end{document}